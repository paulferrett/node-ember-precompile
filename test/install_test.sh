...
TEST_INSTALL_DIR="/tmp/ember-precompilex-test-$(date +%s)"
...
# round-trip-ish test
./node_modules/.bin/ember-precompilex "$TEST_DIR/test1.hbs" > template.js
cat template.js | ./node_modules/.bin/ember-render test1 --context '{"thing": "its and it"}'

# full nested template test
./node_modules/.bin/ember-precompilex "$TEST_DIR/test1.hbs"  "$TEST_DIR/test.two.hbs" > template.js
